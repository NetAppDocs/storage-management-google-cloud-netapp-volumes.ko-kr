...
keywords: google cloud netapp volumes, google cloud, netapp cloud volumes, create netapp cloud volumes, discover netapp cloud volumes service for gcnv, cloud volumes service subscription, discover cloud volumes service subscription, discover region, allowed clients, netapp console, gcnv 
...
+
.. 사용할 시스템 이름을 입력하세요.
.. 연결된 Google 서비스 계정 이메일 주소를 붙여넣으세요. 자세한 내용은 다음을 참조하세요.link:task-set-up-gcnv.html["서비스 계정 설정"] .
.. *자격 증명 적용*을 선택하여 프로젝트 이름과 지역을 검색합니다.
.. *프로젝트 이름*에서 Google Cloud 프로젝트 이름을 선택합니다.
.. *지역*에서 시스템을 만들 지역을 선택합니다. 해당 지역은 Google Cloud NetApp Volumes 에 대해 지원되는 지역 중 하나여야 합니다.
.. *발견*을 선택하세요.
